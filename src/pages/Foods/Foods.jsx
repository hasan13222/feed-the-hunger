...
const Foods = () => {
  ...
  useEffect(() => {
    fetch("https://feed-the-hunger-server.vercel.app/foods")
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      <Helmet>
        <title>FeedTheHunger | Available Foods</title>
      </Helmet>
        ...
        <div className="container mx-auto">
          ...
          <div className="filter flex gap-10 justify-center mb-8">
            <form ref={formRef} onSubmit={handleSearch} className="w-[40%]">
              <input
                className="p-3 mr-1 rounded-md w-[80%]"
                type="text"
                name="search"
            ...
            <button
              onClick={handleLessExpireTime}
              className="sort flex items-center btn bg-green-700 text-white"
            >
              Less Expire Time
              <BiSort />
            </button>
            <button
              onClick={handleMoreExpireTime}
              className="sort flex items-center btn bg-green-700 text-white"
            >
              More Expire Time
              <BiSort />
